...
class HyenaDNAWrapper(mirLM):
    ...
    def run_training(
        ...
        for batch_idx, (seq, seq_mask, seed_start, seed_end, target) in enumerate(train_loader):
            seq, seq_mask, target, seed_start, seed_end = (
                seq.to(self.device), 
    ...
    def run_testing(
        ...
        with torch.no_grad():
            for seq, seq_mask, seed_start, seed_end, target in test_loader:
                ...
                s_w = model.forward(seq=seq, seq_mask=seq_mask, train=False)
                probabilities = s_w.squeeze().sigmoid()
    ...
    def forward(self, 
        ...
        if train:
            ...
            for idx in pos_indices:
                ...
                assert (start.item() != -1) and (end.item() != -1), "During training, seed start and seed end must be provided to perturb the seed region."
