...
# mRNAseq_path = os.path.join(data_dir, "chimpanzee_mrna_seq.txt.gz")
# mRNA_seq_dict = []
# with gzip.open(mRNAseq_path, "rt") as handle:
#     for record in SeqIO.parse(handle, "fasta"):
#         mRNA_seq = str(record.seq) # full-length mRNA seq
#         attrs = record.id.split('|')
#         if mRNA_seq != "Sequenceunavailable" and len(attrs) == 3:
#             gene_id, tran_id, gene_name = record.id.split('|')
...
# mRNA_seq_df.to_csv(
#     os.path.join(os.path.join(data_dir, "chimpanzee_mrna_seq.csv.gz")), 
#     sep='\t', 
#     index=False, 
#     compression='gzip')

# mRNAseq_path = os.path.join(data_dir, "mouse_mrna_seq.txt.gz")
# mRNA_seq_dict = []
# with gzip.open(mRNAseq_path, "rt") as handle:
#     for record in SeqIO.parse(handle, "fasta"):
#         mRNA_seq = str(record.seq) # full-length mRNA seq
