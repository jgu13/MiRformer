...
data_path_train = os.path.join(PROJ_HOME, "TargetScan_dataset/Merged_primates_train_500_randomized_start.csv")
df_train = pd.read_csv(data_path_train, sep=",")
df_train_subset = df_train.sample(n=50000, random_state=42)

df_train_subset.to_csv(
    os.path.join(
        os.path.dirname(data_path_train), f"Merged_primates_train_500_randomized_start_random_samples.csv"
    ),
    sep=",",
    index=False
)
