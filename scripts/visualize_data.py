    ...
    for record in islice(SeqIO.parse(handle, 'fasta'), num_record_to_iter):
        ...
        if miRNA_id.startswith("hsa-miR"): # only store miRNAseq of homo sapiens
            ...
            if miRNA_id not in miRNA_seq_dict:
                ...
                print(f"Sequence for miRNA [{miRNA_id}] is duplicated. Keeping the existing sequence.")
...
with gzip.open(mRNAseq_path, 'rt') as handle:
    for record in islice(SeqIO.parse(handle, 'fasta'), num_record_to_iter):
        ...
        if gene_symbol not in mRNA_seq_dict:
            ...
            print(f"Transcript for gene [{gene_symbol}] is duplicated. Keeping the existing sequence.")
...
with gzip.open(mRNAseq_path, 'rt') as handle:
    for record in SeqIO.parse(handle, 'fasta'):
            ...
            if gene_symbol not in mRNA_seq_dict:
                ...
                if mRNA_seq != "Sequenceunavailable":
                    # Store the sequence
                    mRNA_seq_dict[gene_symbol] = mRNA_seq[:mRNA_length]
                else:
                    print(f"Transcript for gene [{gene_symbol}] is not available.")
            else:
                print(f"Transcript for gene [{gene_symbol}] is duplicated. Keeping the existing sequence.")
...
with open(os.path.join(data_path, f"mRNA {mRNA_length} dictionary.json"), "w") as fp:
    json.dump(mRNA_seq_dict, fp, indent=4)
...
def build_dmiso_dataset(MTI_path, mRNAseq_path, miRNAseq_path):
    ...
    # get mti dataframe
    hsa_mti = pd.read_csv(hsa_MTI_path, compression='gzip')
    hsa_mti_unique = hsa_mti[["miRTarBase ID", "miRNA", "Target Gene"]].drop_duplicates()
            ...
            if miRNA_id.startswith("hsa-miR"): # only store miRNAseq of homo sapiens
                ...
                if miRNA_id not in miRNA_seq_dict["miRNA"]:
                    ...
                    if miRNA_seq != "Sequenceunavailable":
                        ...
                        miRNA_seq_dict["miRNA seq"].append(miRNA_seq) # append miRNA sequence
                    else:
                        print(f"Sequence for miRNA [{miRNA_id}] is not available.")
    ...
    hsa_mti_shuffled["Target Gene"] = np.random.permutation(hsa_mti_unique["Target Gene"].values)
    ...
    # print(len(comb_df))
    comb_df[["miRNA id", "mRNA id", "miRNA seq", "mRNA seq"]].dropna().to_csv("dmiso_pairs.txt.gz", sep="\t", header=False, index=False)
