...
class LinearHead(nn.Module):
    def __init__(self, d_model, d_output=None, hidden_sizes=None):
        super().__init__()
        if hidden_sizes is not None:
            assert isinstance(
                hidden_sizes, list
            ), "hidden_sizes must be a list of integers."
            layers = []
            in_size = d_model
            for h in hidden_sizes:
                ...
                in_size = h
            layers.append(nn.Linear(in_size, d_output)) #bias=False))
        ...
        **kwargs,
        ...
        if self.use_head:
            ...
            self.head = LinearHead(
                d_model=d_model, d_output=n_classes, hidden_sizes=hidden_sizes
            )
    ...
    def forward(
        ...
        if self.use_head:
            if input_mask is not None:
                ...
                # print("hidden_states = ", hidden_states[0,:,0])
            # avg pool on seq_len
            hidden_states_pooled = (
                hidden_states.sum(dim=1) / valid_counts
            )  # (batchsize, hidden_size)
            # # CNN
            # in_channel = hidden_states.shape[2]
            # out_channel = in_channel
            # kernel_size = 10
            # hidden_states = nn.conv1d(in_channel, out_channel, kernel_size=kernel_size, device=device) # (batchsize, 19, hidden_size) where 19 = miRNA_len - 10 + 1 - 1 + 1 = 28-10+1-1+1
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        scratch_model = HyenaDNAModel(
            **config, device=device, use_head=use_head, n_classes=n_classes
