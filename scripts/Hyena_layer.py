...
class HyenaOperator(nn.Module):
    def __init__(
        ...
        self.l_max = l_max
        self.order = order
        inner_width = d_model * (order + 1) # why d_model x (num_recurrence + 1) ?
        self.dropout = nn.Dropout(dropout)
        self.in_proj = nn.Linear(d_model, inner_width) # embedding size -> inner width
    ...
    def forward(self, u, *args, **kwargs): # u = hidden states of layer n
        l = u.size(-2)
        l_filter = min(l, self.l_max)
        u = self.in_proj(u)
        # print("In project shape = ", u.size())
        u = rearrange(u, 'b l d -> b d l')
        # print("After rearrange = ", u.size())

        uc = self.short_filter(u)[...,:l_filter]
        # print("After short filter = ", u.size())
        *x, v = uc.split(self.d_model, dim=1)
        # print(f"After spliting into {self.d_model} chuncks, value shape: ", v.shape)
        ...
        for o, x_i in enumerate(reversed(x[1:])):
            ...
            # print("After HyenaFilter, value shape = ", v.size())
...
class Block(nn.Module):
    ...
    def forward(self, hidden_states, residual = None,
        ...
        if self.prenorm:
            ...
            # print("After Hyena Operator, hidden states shape = ", hidden_states.size())
            if mixer_subset is not None:
...
def create_block(d_model, d_inner=None,
    ...
    mixer_cls = create_mixer_cls(layer=layer,
                                 ...
                                 **factory_kwargs)  # HyenaOperator or MHA creation
...
class LMBackbone(nn.Module):
    ...
    def forward(self, input_ids, position_ids=None):
        ...
        # print("Embedding shape = ", hidden_states.size())
        for layer in self.layers:
            hidden_states, residual = layer(hidden_states, residual)
        # print("After Block, hidden states shape = ", hidden_states.size())
...
class LinearHead(nn.Module):
    def __init__(
        self, d_model, d_output=None, hidden_sizes=None
    ):
        super().__init__()
        if hidden_sizes is not None:
            assert isinstance(hidden_sizes, list), "hidden_sizes must be a list of integers."
            layers = []
            in_size = d_model
            for h in hidden_sizes:
                hidden_layer = nn.Linear(in_size, h)
                layers.append(hidden_layer)
                layers.append(nn.ReLU())
                in_size = h
            layers.append(nn.Linear(in_size, d_output))
            self.output_transform = nn.Sequential(*layers)
        else:    
            self.output_transform = nn.Identity() if d_output is None else nn.Linear(d_model, d_output)
    
    def forward(self, x):
        embedding = self.output_transform(x)
        return embedding        

class SequenceDecoder(nn.Module):
    def __init__(
        self, d_model, d_output=None, l_output=None, use_lengths=False, mode="last", hidden_sizes=None
    ):
        super().__init__()
    ...
    def forward1(self, x, state=None, lengths=None, l_output=None):
        """
...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
        ...
        if self.use_head:
            ...
            hidden_sizes=[d_model*2, d_model*2]
            self.head = LinearHead(d_model=d_model, d_output=n_classes, hidden_sizes=hidden_sizes)
    ...
    def forward(self, device, input_ids, input_mask=None, position_ids=None, state=None): # state for the repo interface
        hidden_states = self.backbone(input_ids, position_ids=position_ids)
        hidden_states = hidden_states.to(device)
        if self.use_head:
            if input_mask is not None:
                # remove padding
                input_mask = input_mask.unsqueeze(-1).expand(-1, -1, hidden_states.shape[2]) # (batchsize, seq_len, hidden_size)
                input_mask = input_mask.to(device)
                hidden_states = hidden_states[input_mask]
            # avg pool on seq_len
            hidden_states_pooled = torch.mean(hidden_states, dim=1) # (batchsize, hidden)
            return self.head(hidden_states_pooled)
        else:
            return hidden_states
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls,
        ...
        scratch_model = HyenaDNAModel(**config, device=device, use_head=use_head, n_classes=n_classes)  # the new model format
